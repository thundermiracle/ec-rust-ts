use crate::application::viewmodels::{ProductListViewModel, ProductSummaryViewModel};
use crate::interface_adapters::products::responses::{
...
impl ProductListPresenter {
    /// ProductListViewModelをProductListResponseに変換
    ///
    /// # Arguments
    /// * `view_model` - アプリケーション層から取得したProductListViewModel
    ///
    /// # Returns
    ...
    /// # Example
    /// ```rust
    /// let response = ProductListPresenter::present(product_list_view_model);
    /// ```
    pub fn present(view_model: ProductListViewModel) -> ProductListResponse {
    ...
    /// ProductSummaryViewModelをProductListItemResponseに変換
    ///
    /// # Arguments
    ...
    fn present_product_item(summary: ProductSummaryViewModel) -> ProductListItemResponse {
        // ViewModelから必要な情報を抽出
...
mod tests {
    ...
    fn test_present_product_list() {
        ...
        let view_model = ProductListViewModel {
    ...
    fn test_present_multiple_products() {
        let product1 = ProductSummaryViewModel::new(
            "product_1".to_string(),
            "商品1".to_string(),
            "家具".to_string(),
            1000,
        ...
        let product2 = ProductSummaryViewModel::new(
            "product_2".to_string(),
            "商品2".to_string(),
            "雑貨".to_string(),
            2000,
