...
pub trait ProductDetailService {
    /// IDで詳細商品情報を取得
    async fn get_detailed_product(&self, id: u32) -> Result<Option<ProductDetail>, RepositoryError>;
    
    /// 全ての詳細商品情報を取得
    async fn get_all_detailed_products(&self) -> Result<Vec<ProductDetail>, RepositoryError>;
    
    /// カテゴリー別詳細商品情報を取得
    async fn get_detailed_products_by_category(&self, category_id: u32) -> Result<Vec<ProductDetail>, RepositoryError>;
    
    /// ベストセラー商品を取得
    async fn get_best_seller_products(&self) -> Result<Vec<ProductDetail>, RepositoryError>;
    
    /// セール商品を取得
    async fn get_on_sale_products(&self) -> Result<Vec<ProductDetail>, RepositoryError>;
    
    /// 迅速配送商品を取得
    async fn get_quick_ship_products(&self) -> Result<Vec<ProductDetail>, RepositoryError>;
    
    /// 商品検索（名前で部分一致）
    async fn search_products(&self, query: &str) -> Result<Vec<ProductDetail>, RepositoryError>;
    ...
    async fn get_products_paginated(
        ...
        page: u32,
        per_page: u32,
    ) -> Result<(Vec<ProductDetail>, u64), RepositoryError>; // (products, total_count)
}
...
#[async_trait::async_trait]
pub trait AdvancedProductDetailService: ProductDetailService {
    ...
    async fn get_products_advanced(
        ...
        page: u32,
        per_page: u32,
    ) -> Result<(Vec<ProductDetail>, u64), RepositoryError>;
    
    /// 関連商品を取得（同カテゴリー、類似価格帯）
    async fn get_related_products(&self, product_id: u32, limit: u32) -> Result<Vec<ProductDetail>, RepositoryError>;
    
    /// おすすめ商品を取得
    async fn get_recommended_products(&self, limit: u32) -> Result<Vec<ProductDetail>, RepositoryError>;
}
